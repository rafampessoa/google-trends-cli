...
from gtrends.config import DEFAULT_REGION, DEFAULT_TIMEFRAME, DEFAULT_CATEGORY
...
class TrendsClient:
    ...
    def get_trending_searches_with_articles(self,
                                        region: Optional[str] = None,
                                        limit: int = 20) -> Tuple[pd.DataFrame, Dict]:
        ...
        for i, trend in enumerate(trending[:limit]):
            data.append({
                'rank': i+1,
                'title': trend.keyword,  # Changed from trend.title to trend.keyword
                'traffic': trend.volume if hasattr(trend, 'volume') else '',  # Changed to volume
                'raw_data': trend  # Store the raw object for later access
            })
    ...
    def get_related_topics(self, 
                         ...
                         category: str = DEFAULT_CATEGORY) -> Dict[str, pd.DataFrame]:
        ...
        # Get related topics
        related = self.trends.related_topics(
            query, 
            timeframe=timeframe, 
            geo=region, 
            cat=category
    ...
    def get_interest_over_time(self,
                             ...
                             category: str = DEFAULT_CATEGORY) -> pd.DataFrame:
    ...
    def get_interest_by_region(self,
                             ...
                             resolution: str = "COUNTRY") -> pd.DataFrame:
    ...
    def get_showcase_timeline(self,
                           keywords: List[str],
        ...
        timeline_df = self.trends.trending_now_showcase_timeline(
            keywords,
