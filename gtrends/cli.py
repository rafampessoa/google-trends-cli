...
def related(query: str, region: Optional[str], timeframe: str, category: str, 
    ...
    try:
        ...
        console.print(f"[bold]Related data for '{query}' in {region_name} over {timeframe_parsed}[/bold]\n")
...
def compare(topics, region: Optional[str], timeframe: str, category: str,
    ...
    try:
        if not topics:
            console.print("[bold red]Error:[/bold red] Please provide at least one topic to compare.")
            return
            
        if len(topics) > 5:
            console.print("[bold yellow]Warning:[/bold yellow] Google Trends supports comparing max 5 topics. Using first 5.")
            topics = topics[:5]
...
def writing_opportunities(region: Optional[str], timeframe: str, count: int,
    ...
    try:
        ...
        if opportunities_df.empty:
            console.print("[yellow]No writing opportunities found for the given parameters.[/yellow]")
            return
...
def topic_growth(topics, period: str, export: bool, export_path: Optional[str], format: str, visualize: bool):
    ...
    try:
        if not topics:
            console.print("[bold red]Error:[/bold red] Please provide at least one topic to analyze.")
            return
        ...
        if len(topics) > 20:  # Show warning but don't truncate (TrendsPy can handle hundreds)
            console.print("[bold yellow]Warning:[/bold yellow] Analyzing many topics. This may take a while.")
...
def geo_interest(query: str, region: Optional[str], resolution: str, timeframe: str,
    ...
    try:
        ...
        if export:
            export_dir = validate_export_path(export_path)
            export_file = export_dir / f"geo_interest_{query}_{region_display}_{resolution}.{format}"
            export_to_file(interest_df, export_file, format)
