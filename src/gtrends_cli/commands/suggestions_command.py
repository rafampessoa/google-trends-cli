...
@click.option(
    "--timeframe", "-t", default="today 7-d", help="Timeframe for data (e.g., 'today 7-d', 'today 1-m')"
)
@click.option(
    "--count",
    "-n",
...
def suggest_topics_command(
    ...
    try:
        ...
        console.print(
            f"[bold]Topic suggestions for {category} content in {region_name} over {timeframe_parsed}[/bold]\n"
        )
