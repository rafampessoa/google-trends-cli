...
def model_to_dataframe(model: BaseModel) -> Union[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    if isinstance(model, TrendingSearchResults):
        ...
        return pd.DataFrame([{
            ...
            "timeframe": model.timeframe if isinstance(model.timeframe, str) else str(model.timeframe),
        ...
        try:
            ...
            for key, value in data.items():
                ...
                elif isinstance(value, dict) and value and any(isinstance(v, BaseModel) for v in value.values()):
                    data[key] = {k: v.to_dict() if isinstance(v, BaseModel) else v for k, v in value.items()}
