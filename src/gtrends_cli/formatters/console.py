...
def format_trending_searches(results: TrendingSearchResults, count: int = 10) -> None:
    ...
    if any(hasattr(topic, 'volume_growth_pct') and topic.volume_growth_pct is not None for topic in results.topics):
    ...
    for i, topic in enumerate(results.topics[:count]):
        ...
        if hasattr(table, 'columns') and len(table.columns) > 2:
            ...
            volume_str = f"{topic.volume:,}" if hasattr(topic, 'volume') and topic.volume is not None else ""
            row.append(volume_str)
        
        # Add growth percentage if column exists
        if hasattr(table, 'columns') and len(table.columns) > 3:
            growth_str = f"{topic.volume_growth_pct:+.1f}%" if hasattr(topic, 'volume_growth_pct') and topic.volume_growth_pct is not None else ""
    ...
    for i, topic in enumerate(results.topics[:count]):
        ...
        has_details = (
            (hasattr(topic, 'trend_keywords') and topic.trend_keywords) or
            (hasattr(topic, 'topics') and topic.topics) or
            (hasattr(topic, 'news') and topic.news)
        )
    ...
    if results.has_news and not any(hasattr(topic, 'news') and topic.news for topic in results.topics):
...
def format_related_data(
    results: Union[RelatedTopicResults, RelatedQueryResults], 
    count: int = 10
) -> None:
...
def format_interest_over_time(results: InterestOverTimeResult) -> None:
    ...
    for topic, points in results.time_series.items():
        ...
        date_range = f"{start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}" if start_date and end_date else "N/A"
