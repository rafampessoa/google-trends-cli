...
class TrendingTopicResponse(BaseModel):
    ...
    volume_growth_pct: Optional[float] = Field(None, description="Percentage growth in search volume")
    ...
    class Config:
        json_schema_extra = {"examples": [{"keyword": "example topic", "rank": 1, "volume": 5000, "volume_growth_pct": 120.5, "geo": "US"}]}
...
class TrendingSearchResponse(BaseModel):
    ...
    news_articles: Optional[Dict[str, List[NewsArticleResponse]]] = Field(None, description="News articles by topic")
...
class RelatedTopicResponse(BaseModel):
    ...
    rising_value_text: Optional[str] = Field(None, description="Rising value text (e.g., 'Breakout')")
...
class RelatedDataResponse(BaseModel):
    ...
    timeframe: str = Field(..., description="Timeframe used")
    category: str = Field(..., description="Category used")
    topics: Dict[str, List[RelatedDataItem]] = Field(..., description="Related topics (top and rising)")
    queries: Dict[str, List[RelatedDataItem]] = Field(..., description="Related queries (top and rising)")
...
class InterestOverTimeResponse(BaseModel):
    ...
    category: str = Field(..., description="Category used")
    time_series: Dict[str, List[TimePointResponse]] = Field(..., description="Time series data per topic")
...
class InterestByRegionResponse(BaseModel):
    ...
    region_interest: Dict[str, List[RegionInterestResponse]] = Field(..., description="Region interest data per topic")
...
class GrowthItem(BaseModel):
    ...
    data_points: Optional[List[TimePointResponse]] = Field(None, description="Historical data points")
