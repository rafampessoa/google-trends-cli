...
class TrendingRequest(BaseModel):
    ...
    include_news: bool = Field(
        False, 
        description="Whether to include related news articles"
    )
...
class RelatedRequest(BaseModel):
    ...
    query: str = Field(
        ..., 
        description="Search term to find related topics/queries"
    )
    region: Optional[str] = Field(
        None, 
        description="Two-letter country code (e.g., US, GB, AE). Auto-detects if not specified."
    ...
    timeframe: str = Field(
        "today 1-d", 
        description="Time range for data (e.g., 'today 1-d', 'today 3-m')"
    )
    category: str = Field(
        "0", 
...
class ComparisonRequest(BaseModel):
    ...
    timeframe: str = Field(
        "today 3-m", 
        description="Time range for data (e.g., 'today 3-m', 'today 12-m')"
    )
    category: str = Field(
        "0", 
        description="Category ID to filter results"
...
class InterestByRegionRequest(BaseModel):
    ...
    topics: List[str] = Field(
        ..., 
        description="List of topics to analyze",
        min_items=1,
        max_items=5
    )
    region: Optional[str] = Field(
    ...
    timeframe: str = Field(
        "today 12-m", 
        description="Time range for data (e.g., 'today 12-m')"
    )
    category: str = Field(
        "0", 
        description="Category ID to filter results"
    ...
    resolution: ResolutionEnum = Field(
        ResolutionEnum.COUNTRY, 
        description="Geographic resolution level"
    )
    limit: int = Field(
        20, 
        description="Maximum number of regions to return per topic",
        ge=1,
        le=100
    )
...
class TopicGrowthRequest(BaseModel):
    ...
    topics: List[str] = Field(
        ..., 
        description="List of topics to analyze growth for",
        min_items=1,
        max_items=50
    )
    period: BatchPeriodEnum = Field(
        BatchPeriodEnum.PAST_24H, 
...
class GrowthRequest(BaseModel):
    ...
    topics: List[str] = Field(
        ..., 
        description="List of topics to analyze growth",
        min_items=1,
        max_items=10
    )
    period: str = Field(
        "24h", 
        description="Time period to analyze (4h, 24h, 48h, 7d)"
    )
...
class SuggestionRequest(BaseModel):
    ...
    category: str = Field(
        "books", 
        description="Content category (books, news, arts, fiction, culture)"
    )
    region: Optional[str] = Field(
        None, 
        description="Two-letter country code (e.g., US, GB, AE). Auto-detects if not specified."
...
class GeoInterestRequest(BaseModel):
    ...
    query: str = Field(
        ..., 
        description="Search term to analyze geographic interest"
    )
    region: Optional[str] = Field(
        None, 
        description="Two-letter country code (e.g., US, GB, AE). Auto-detects if not specified."
    ...
    count: int = Field(
        20, 
        description="Number of regions to display",
        ge=1,
        le=100
    )
...
class OpportunityRequest(BaseModel):
    ...
    seed_topics: Optional[List[str]] = Field(
        None, 
        description="Optional list of seed topics to base suggestions on"
    )
    region: Optional[str] = Field(
        None, 
        description="Two-letter country code (e.g., US, GB, AE). Auto-detects if not specified."
    ...
    timeframe: str = Field(
        "today 1-m", 
        description="Time range for data (e.g., 'today 1-m', 'today 3-m')"
    )
    count: int = Field(
        5, 
        description="Number of opportunities to find",
        ge=1,
        le=20
    ) 