...
from gtrends_core.utils.validators import validate_region_code, parse_timeframe
...
@router.get("/interest/{query}", response_model=GeoInterestResponse, summary="Get geographical interest data")
async def get_geo_interest(
    query: str = Path(..., description="Search query to analyze geographical interest for"),
    region: Optional[str] = Query(None, description="Region code (e.g., US, GB). Auto-detects if not specified."),
    resolution: str = Query("COUNTRY", description="Geographic resolution level (COUNTRY, REGION, CITY, DMA)"),
    ...
    This endpoint provides data on relative interest in a search term across different geographic regions.
...
@router.post("/interest", response_model=GeoInterestResponse, summary="Get geographical interest data")
...
@router.get("/codes/{search_term}", response_model=RegionCodeResponse, summary="Search for region codes")
