...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, 
        app, 
        rate_limit_per_minute: int = API_RATE_LIMIT,
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        response.headers["X-RateLimit-Remaining"] = str(self._get_remaining(client_id, current_time))
    ...
    def _cleanup_old_requests(self, client_id: str, current_time: float) -> None:
        ...
        self.requests[client_id] = {
            ts: count for ts, count in self.requests[client_id].items() 
            if ts > cutoff
        }
