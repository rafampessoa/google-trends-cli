        ...
        result['news'] = [
            {
                ...
                'url': article.url,
                'time': article.time.isoformat() if hasattr(article.time, 'isoformat') else article.time,
...
def export_to_file(
    ...
    try:
        ...
        elif isinstance(data, list) and len(data) > 0 and hasattr(data[0], '__class__') and data[0].__class__.__name__ == 'TrendingTopic':
        ...
        # Handle other list types
            ...
            raise ExportException(
                "Unsupported data type for export", 
                file_path=str(file_path), 
                format=format
            )
