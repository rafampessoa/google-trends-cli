...
def mock_trends_client():
    ...
    client.get_trending_searches_with_articles.return_value = (pd.DataFrame(trending_data), news_articles)
    ...
    region_data = {
        'query1': [100, 90, 80, 70, 60],
        'query2': [50, 60, 70, 80, 90]
    }
...
def mock_content_suggester(mock_trends_client):
    ...
    with patch.object(ContentSuggester, 'suggest_topics', 
                     return_value=pd.DataFrame(topics_data)):
        ...
        with patch.object(ContentSuggester, 'get_writing_opportunities', 
